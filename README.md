...
- 月视图补齐完整自然周；日期角标只统计当天未完成待办和尚未开始的会议，已完成待办及已开始会议不计入。过去日期存在未完成待办时使用柔和橙色角标，其余角标保持蓝色。
- 启动、切换周/月视图及紧凑/展开模式时自动定位系统当天；应用跨日或重新回到前台后，周、月和待办同步切换到当前周期与当天。
...
- 采用 iOS Liquid Glass 视觉系统：窗口、导航、控件组和弹层使用共享玻璃材质，交互控件提供轻量指针反馈和形变过渡，内容区保持清晰；支持深浅色、高对比度、减少动态、降低透明度以及无背景模糊能力回退。
- 设置使用四档滑杆调节字体，当前标准字号为第 1 档，后续三档逐级放大；界面不额外显示档位百分比提示，点击设置栏外部时自动收起。
