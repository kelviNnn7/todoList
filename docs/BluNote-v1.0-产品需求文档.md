...
| I-036 | v0.3.4 外观状态优化 | 移除字体大小右侧提示；逾期待办在日历上改用符合整体审美的橙色背景角标；更新后推送 main 并生成 main DMG | 隐藏可见档位百分比并保留读屏值；角标模型增加逾期状态，浅色使用低饱和橙色、深色使用系统橙色；完成全量回归后发布 main | FR-026、FR-028、FR-030、FR-019 | 已实现并获准发布 main；待 main CI、DMG 完整性和真机视觉验收 |
...
|---|---|---|---|---|
| macOS | Apple Silicon；Intel 策略待确认 | DMG | v0.3.3 arm64 已通过 CI 构建与完整性校验 | Developer ID 签名、公证、Gatekeeper、多机验证 |
