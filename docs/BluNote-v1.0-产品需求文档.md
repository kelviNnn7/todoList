...
以 dev v0.3.5 为当前开发基线、main v0.3.4 为稳定发布基线，持续建设一个简洁、可拖拽、可缩放、可调整透明度和字体大小的桌面任务与会议中心。v1.0 在保留轻量挂件体验的同时，完成任务全生命周期、可靠提醒、搜索整理、重复计划、备份恢复、三平台一致性和可发布安装链路。本地离线使用是默认能力，账号、云同步和团队协作不进入 v1.0。
...
- 本次从 `main` 构建 macOS v0.3.4 Apple Silicon DMG；Windows 当前安装包为 v0.3.2 x64 NSIS。其他 Mac 正式分发仍需 Developer ID 签名、公证和相应架构构建。
- macOS 与 Windows 必须运行同一套共享界面与领域逻辑自动化；Windows 还需通过原生编译、启动冒烟和安装包完整性校验。
- 麒麟 OS v0.3.2 使用精简后的自包含 Electron 运行时，不依赖 `libwebkit2gtk-4.1-0`，支持银河麒麟 V10 SP1 2503 与 Hygon C86 x86_64。
...
- 以 `affaan-m/ECC` 的 `liquid-glass-design` Skill 为本次设计执行规范，并结合现有 React/Tauri、Electron 与 WidgetKit 技术边界实现等价效果。
...
- 不支持 `backdrop-filter` 的 Windows/麒麟运行环境自动使用不透明回退；三平台保持相同结构、信息层级和交互语义。
...
| FR-005 | 周/月日历、周一至周日规则与系统日期自动同步 | P0 | 已实现 | 三平台休眠恢复与时区边界回归 |
...
| FR-021 | 麒麟 OS 自包含 DEB | P0 | 已实现 | v0.3.2 已裁剪重复依赖和语言资源；指定真机完整验收 |
...
| FR-026 | 四档字体滑杆调节与持久化，隐藏右侧档位提示 | P1 | 已实现 | 已保留无障碍档位信息；补充三平台视觉回归 |
| FR-027 | 点击设置栏外部自动收起 | P1 | 已实现 | 补充键盘交互回归 |
| FR-028 | 日历角标仅统计未完成待办和未开始会议，逾期待办使用橙色 | P0 | 已实现 | 补充三平台真机时间与颜色对比度回归 |
| FR-029 | 启动、跨日、前台恢复及视图切换时自动定位当前周/月与当天待办 | P0 | 已实现 | 补充三平台休眠恢复真机回归 |
| FR-030 | 逾期待办与普通日历角标使用橙色/蓝色状态区分 | P1 | 已实现 | 补充深浅色与高对比度真机回归 |
| FR-031 | iOS Liquid Glass 全界面材质、交互反馈、Widget 适配及无障碍回退 | P1 | 已实现 | 补充三平台不同桌面背景与系统辅助模式真机视觉回归 |

### Iteration History

下表记录截至 2026-09-07 的全部已知需求迭代。相同主题的追问和缺陷复测保留为独立条目，以便追踪需求演进。
...
|---|---|---|---|---|
| macOS | Apple Silicon；Intel 策略待确认 | DMG | v0.3.4 arm64 已从 main 通过 CI 构建与完整性校验 | Developer ID 签名、公证、Gatekeeper、多机验证 |
| Windows | Windows 10/11 x64 | NSIS EXE | v0.3.2 x64 已通过 CI 构建、启动冒烟与安装包校验 | Authenticode 签名、升级/卸载与真机视觉验证 |
| 银河麒麟 | V10 SP1 2503，Hygon C86/x86_64 | DEB | v0.3.2 精简自包含运行时，97,729,360 字节 | 指定真机安装、启动、窗口、通知、持久化长稳测试 |
...
当前 dev v0.3.5 基线已有 50 项前端自动化测试和 6 项 Rust 测试。v1.0 测试矩阵必须包括：
