...
以 dev v0.3.2 为当前开发基线，持续建设一个简洁、可拖拽、可缩放、可调整透明度和字体大小的桌面任务与会议中心。v1.0 在保留轻量挂件体验的同时，完成任务全生命周期、可靠提醒、搜索整理、重复计划、备份恢复、三平台一致性和可发布安装链路。本地离线使用是默认能力，账号、云同步和团队协作不进入 v1.0。
...
- 本次同步构建 macOS v0.3.2 Apple Silicon DMG 与 Windows v0.3.2 x64 NSIS；其他 Mac 正式分发仍需 Developer ID 签名、公证和相应架构构建。
...
当前 dev v0.3.2 基线已有 45 项前端自动化测试和 6 项 Rust 测试。v1.0 测试矩阵必须包括：
