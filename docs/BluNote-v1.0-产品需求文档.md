...
> 状态定义：**已实现**表示代码基线已具备；**部分实现**表示已有基础能力但仍存在发布差距；**待实现**表示 v1.0 规划需求；**已撤销**表示历史上实现过但已明确移出现行范围；**工程约束**表示后续开发必须持续遵守。
...
当前主要问题包括：提醒仍依赖应用运行时轮询；macOS 正式签名、公证与多架构分发尚未闭环；WidgetKit 需要完整 Xcode 与签名环境验证；搜索、重复计划、备份恢复等高频效率能力尚未实现。v0.3.6 起 `dev` 收敛为仅支持 macOS，已删除其他平台的运行时、构建链路和发布产物。

### Proposed Solution

以 dev v0.3.6 为当前开发基线、main v0.3.4 为稳定发布基线，持续建设一个简洁、可拖拽、可缩放、可调整透明度和字体大小的 macOS 任务与会议中心。v1.0 在保留轻量挂件体验的同时，完成任务全生命周期、可靠提醒、搜索整理、重复计划、备份恢复和可发布的 macOS 安装链路。本地离线使用是默认能力，账号、云同步和团队协作不进入 v1.0。

### Success Criteria

| 指标 | v1.0 验收目标 | 验证方式 |
|---|---:|---|
| P0 功能 | 验收用例通过率 100% | CI 与 macOS 手工回归 |
...
| 稳定性 | 72 小时长稳测试无 P0/P1 缺陷 | macOS 真机测试 |
...
4. **macOS 多设备用户**：使用 Apple Silicon 或 Intel Mac，希望安装、窗口与数据体验一致。
...
- 应用启动时，周视图自动展示系统日期所在周，月视图自动展示系统日期所在月，待办列表自动选择系统当天。
- 系统日期跨日或应用从后台恢复到前台时，周/月日历与待办列表自动同步至新的系统当天；切换周/月或紧凑/展开视图时同样回到当前周期和当天。
- 用户仍可在当前日期内手动浏览上一周期、下一周期和其他日期，自动同步不得在未跨日时打断浏览。
- 日期角标只统计当天未完成待办和尚未开始的会议；已完成待办、已开始会议均不计入，数量大于 9 时显示 9。
- 过去日期只要存在未完成待办，数量角标使用低饱和橙色背景；非逾期日期角标保持蓝色，已完成待办不得触发橙色状态。
- 会议是否“未开始”以当前时间早于会议开始时间为准；切换日期后列表同步更新。
...
- 支持桌面挂件模式、置顶状态和窗口锁定。
- 字体大小继续使用四档滑杆调节，但不在“字体大小”右侧显示档位或百分比提示；无障碍读屏信息必须保留。
...
- macOS 使用 SQLite `todo.db`；浏览器预览环境使用 `todo.*` 本地键。
- 应用不要求登录，不默认上传任务、会议或使用数据。
- v1.0 需要持续维护版本化数据库迁移，并覆盖升级数据完整性验证。
- 数据文件损坏时不得静默覆盖，应保留原文件并引导恢复。

#### US-16 macOS 安装与升级（P0，部分实现）

As a macOS 用户, I want to 点击安装包即可安装和启动 so that 不需要使用命令行。

Acceptance Criteria:

- macOS 提供可双击安装的 DMG。
- `main` 当前稳定包为 v0.3.4 Apple Silicon DMG；`dev` v0.3.6 仅保留 macOS 构建链路。
...
- 滑杆按整数档位吸附，界面仅展示 1–4 档刻度，不在标题右侧显示档位或百分比；读屏信息包含当前档位与百分比。
- 字体大小作用于主界面、事项卡片、日历、设置栏和新增/编辑表单，并即时生效。
- 字体设置保存到本机，应用重启后保持。
- 点击设置栏内部不会关闭；点击设置栏与触发按钮之外的任意界面位置时自动收起。
- 设置项均即时保存，自动收起不会丢失更改。

#### US-22 iOS Liquid Glass 全界面视觉系统（P1，已实现）

As a 重视系统审美的桌面用户, I want to 使用具有 iOS 液态玻璃质感且保持清晰的界面 so that 挂件能自然融入桌面并提供一致的交互反馈。

Acceptance Criteria:

- 以 `affaan-m/ECC` 的 `liquid-glass-design` Skill 为本次设计执行规范，并结合现有 React/Tauri 与 WidgetKit 技术边界实现等价效果。
- 窗口外壳、顶部工具组、事项筛选、周/月切换、设置菜单、分段控件和新增/编辑弹层使用统一的半透明材质、细描边、镜面高光、背景模糊与柔和阴影。
- 同一组相邻控件共享一个玻璃容器，子控件不得各自重复叠加背景模糊；待办、会议和日历正文使用清晰的内容材质。
- 仅按钮、开关、日期、任务卡等可交互元素提供悬停、按压或轻量形变反馈；非交互装饰不得制造虚假点击感。
- 设置菜单、弹层、事项详情和每周日期选择的出现/消失使用短时连续过渡，并在“减少动态效果”模式下降级为近即时完成。
- 主操作使用系统蓝色强调，会议和逾期状态仅使用对应语义色；普通玻璃层保持中性，避免无意义彩色装饰。
- 外层与内层圆角保持同心关系，紧凑态 360 × 620 和展开态 900 × 640 均不得溢出、遮挡或改变原有左右 50% 布局。
- 浅色、深色与高对比度模式下文字、焦点、选中态和边界清晰可辨；系统启用“降低透明度”时停用模糊并使用不透明材质。
- WebView 不支持 `backdrop-filter` 或系统开启“降低透明度”时，自动使用不透明回退并保持信息层级和交互语义。
- WidgetKit 使用系统管理的容器背景、环境自适应明暗材质和语义会议卡；不得依赖 iOS 26 专属 API 而破坏既有 macOS 13 最低版本。

### Functional Requirement Matrix

| ID | 功能需求 | 优先级 | 当前状态 | v1.0 发布差距 |
|---|---|---:|---|---|
| FR-001 | 紧凑/展开双视图 | P0 | 已实现 | 补充 macOS 视觉回归 |
| FR-002 | 展开态左右 50% 布局与列表滚动 | P0 | 已实现 | 补充不同尺寸回归 |
| FR-003 | 窗口拖动、八向缩放、锁定、吸附和可视边界 | P0 | 已实现 | macOS 多屏验证 |
| FR-004 | 冷启动正确缩放 | P0 | 已实现 | 多缩放比例真机验证 |
| FR-005 | 周/月日历、周一至周日规则与系统日期自动同步 | P0 | 已实现 | macOS 休眠恢复与时区边界回归 |
...
| FR-015 | 托盘、自启动与单实例 | P1 | 部分实现 | macOS 语义与回归 |
| FR-016 | WidgetKit 小/中组件 | P1 | 部分实现 | 正式签名和真机验证 |
| FR-017 | BluNote 名称与最新图标全触点统一 | P0 | 已实现 | 安装升级缓存回归 |
| FR-018 | 本地持久化与数据库迁移 | P0 | 部分实现 | 版本化迁移与损坏恢复 |
| FR-019 | macOS DMG | P0 | 部分实现 | 签名、公证、arm64/x64 策略 |
| FR-020 | Windows 图形安装包 | P0 | 已撤销 | v0.3.6 已从 dev 删除实现与构建链路；历史记录保留 |
| FR-021 | 麒麟 OS 自包含 DEB | P0 | 已撤销 | v0.3.6 已从 dev 删除实现与构建链路；历史记录保留 |
...
| FR-026 | 四档字体滑杆调节与持久化，隐藏右侧档位提示 | P1 | 已实现 | 已保留无障碍档位信息；补充 macOS 视觉回归 |
| FR-027 | 点击设置栏外部自动收起 | P1 | 已实现 | 补充键盘交互回归 |
| FR-028 | 日历角标仅统计未完成待办和未开始会议，逾期待办使用橙色 | P0 | 已实现 | 补充 macOS 真机时间与颜色对比度回归 |
| FR-029 | 启动、跨日、前台恢复及视图切换时自动定位当前周/月与当天待办 | P0 | 已实现 | 补充 macOS 休眠恢复真机回归 |
| FR-030 | 逾期待办与普通日历角标使用橙色/蓝色状态区分 | P1 | 已实现 | 补充深浅色与高对比度真机回归 |
| FR-031 | iOS Liquid Glass 全界面材质、交互反馈、Widget 适配及无障碍回退 | P1 | 已实现 | 补充 macOS 不同桌面背景与系统辅助模式真机视觉回归 |

### Iteration History

下表记录截至 2026-09-09 的全部已知需求迭代。相同主题的追问和缺陷复测保留为独立条目，以便追踪需求演进。
...
| I-033 | v0.3.2 | 日历小数字仅显示未完成待办和未开始会议；同步交付 macOS 与 Windows 安装包并严格回归 | 统一角标业务口径；共享自动化在 macOS/Windows 分别执行；Windows 增加应用启动与 NSIS 完整性校验 | FR-005、FR-019、FR-020、FR-028 | 双平台 CI 已通过，已获批准发布至 main；真机安装/视觉验收继续作为正式分发门禁 |
| I-034 | v0.3.2 麒麟包优化 | 在不恢复 WebKitGTK 依赖的前提下缩小麒麟安装包 | DEB 启用最高压缩；仅保留中文语言包；移除 Vite 已打包后重复携带的运行时 `node_modules`；保留软件渲染和高 DPI 资源 | FR-021 | dev 已实现；结构、架构、依赖与可解包校验通过，待指定真机验收 |
| I-035 | v0.3.3 日期自动定位 | 周/月界面跟随系统日期展示当前周和当前月，待办自动切换至当日 | 启动、跨日、前台恢复、周/月切换及紧凑/展开切换时统一定位系统当天；当前日期内保留手动浏览能力 | FR-005、FR-029 | dev 已实现；自动化覆盖周界、月界和视图切换回当天；macOS/Windows CI 通过，arm64 DMG 已校验，待三平台真机验收 |
| I-036 | v0.3.4 外观状态优化 | 移除字体大小右侧提示；逾期待办在日历上改用符合整体审美的橙色背景角标；更新后推送 main 并生成 main DMG | 隐藏可见档位百分比并保留读屏值；角标模型增加逾期状态，浅色使用低饱和橙色、深色使用系统橙色；完成全量回归后发布 main | FR-026、FR-028、FR-030、FR-019 | 已发布 main；前端、安全审计、macOS/Windows 原生 CI 全部通过，arm64 DMG 完整性已校验，待多机视觉验收 |
| I-037 | v0.3.5 iOS Liquid Glass 全界面迭代 | 先将 main 精确复制到 dev，再使用 `affaan-m/ecc@liquid-glass-design` Skill 优化所有 UI | dev 已重置并同步 main；按共享玻璃容器、交互反馈、连续形变、减少嵌套和明暗/无障碍验证原则重构主窗口、导航、日历、事项卡、设置、表单、提示及 WidgetKit | FR-014、FR-016、FR-019、FR-031 | dev 已实现；紧凑态、展开态、设置和弹层截图及控制台通过，50 项前端测试、6 项 Rust 测试、Swift 语法解析、依赖审计和 arm64 DMG 完整性/包内签名结构校验通过；WidgetKit 完整类型检查与扩展编译受本机 Xcode/SDK 环境阻断，待完整 Xcode 和真机验证，未经批准不发布 main |
| I-038 | v0.3.6 macOS-only 收敛 | 删除 dev 中全部 Windows 与麒麟实现，并清除旧产品名相关代码和注释 | 删除 Electron 运行时、DEB/NSIS 构建配置、平台专属图标与 CI；桌面桥接收敛为 Tauri；移除旧命名空间和数据库迁移分支；当前支持范围改为 macOS | FR-018、FR-019、FR-020、FR-021 | dev 已实现；48 项前端测试、5 项 Rust 测试、Clippy、Swift 语法解析、依赖审计和 arm64 DMG 挂载/版本/签名校验通过；待 main 批准 |
...
| v1.0-r5 | 2026-08-27 | v0.3.2 | 记录日历角标统计口径、macOS/Windows 同步安装包与双平台严格回归要求 |
| v1.0-r6 | 2026-08-27 | v0.3.2 | 记录 v0.3.2 获准同步至 main，并保留正式签名与真机验收要求 |
| v1.0-r7 | 2026-08-28 | v0.3.2 | 记录麒麟 DEB 体积优化方案、量化结果与兼容性保留边界 |
| v1.0-r8 | 2026-09-07 | v0.3.3 | 记录周/月日历和待办随系统日期自动定位当前周期与当天的功能迭代 |
| v1.0-r9 | 2026-09-07 | v0.3.4 | 记录字体提示精简、逾期橙色日历角标与 main DMG 发布要求 |
| v1.0-r10 | 2026-09-07 | v0.3.4 | 记录 v0.3.4 main 发布、双平台 CI 与 DMG 完整性验证结果 |
| v1.0-r11 | 2026-09-07 | v0.3.5 | 记录基于指定 liquid-glass-design Skill 的全界面材质、交互、WidgetKit 与跨平台回退迭代 |
| v1.0-r12 | 2026-09-09 | v0.3.6 | 记录 dev 收敛为 macOS-only、撤销 Windows/麒麟交付要求及清除旧命名兼容代码 |
...
- Windows、Linux 或其他非 macOS 平台的运行时、安装包与兼容层。
...
```mermaid
flowchart TD
    UI["React / TypeScript UI"] --> Bridge["Tauri API 封装"]
    Bridge --> Tauri["Tauri + Rust（macOS）"]
...
| Tauri API 封装 | 窗口、持久化、通知和系统能力抽象 | 页面通过职责接口访问原生能力 |
| Tauri/Rust | macOS 原生窗口、SQLite 和系统集成 | 完成正式分发、安全配置与稳定迁移 |
...
|---|---|---|---|---|
| macOS | Apple Silicon；Intel 策略待确认 | DMG | main v0.3.4；dev v0.3.6 已收敛为 macOS-only | Developer ID 签名、公证、Gatekeeper、多机验证 |
...
| 阶段 | 目标 | 核心交付 | 退出条件 |
|---|---|---|---|
| A：v0.3.6 基线固化 | 验证当前能力 | macOS 功能清单、自动化与安装产物证据 | 当前 P0 无阻断缺陷 |
...
| D：v0.9 发布候选 | 完成 macOS 工程化 | macOS 签名公证、Widget 真机和版本化迁移 | macOS 安装和 72 小时长稳通过 |
...
| 风险 | 影响 | 缓解措施 |
|---|---|---|
| SQLite 与浏览器预览存储并存 | 预览和桌面行为可能分叉 | 建立统一仓储契约与版本化模型测试 |
...
- 前端、Rust 与 macOS 原生检查全部通过，代码评审无阻断问题。
- 命名扫描确认内部新增代码不包含具体产品命名，历史兼容逻辑保持隔离。
- macOS 全新安装、覆盖升级、卸载和数据保留验证通过。
- macOS 签名、公证与目标架构验证通过。
