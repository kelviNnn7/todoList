...
- [ ] macOS、Windows、麒麟 x86_64 与麒麟 ARM64 GitHub Actions 原生构建通过
- [ ] 麒麟 `.deb` 元数据、依赖关系与 AppImage 架构检查通过
- [ ] WidgetKit `.appex` 编译、嵌入、App Group entitlement 与深链校验通过
- [ ] 安装包解压体积与运行内存符合 KR1

## 手工平台矩阵

| 场景 | macOS 13/14/15 | Windows 10/11 | 麒麟 V10/openKylin X11 | 麒麟/openKylin Wayland |
|---|---:|---:|---:|---:|
| 首次启动、透明窗口、浅/深色 | 待签名构建 | 待 CI 构建 | 待真机 | 待真机 |
| 多显示器位置恢复与锁定 | 待真机 | 待真机 | 待真机 | 受合成器限制 |
| 关闭转托盘、托盘退出、单实例 | 待真机 | 待真机 | 待真机 | 待真机 |
| 通知授权、睡眠唤醒、时区/DST | 待真机 | 待真机 | 待真机 | 待真机 |
| 通知快捷稍后与每日 3 次上限 | 待真机 | 待真机 | 待真机 | 待真机 |
| 桌面挂件、透明度、拖动与缩放 | 待真机 | 待真机 | 待真机 | 层级/定位由系统管理 |
| 安装、升级、卸载与数据保留 | 待签名构建 | 待签名构建 | 待真机 | 待真机 |
...
3. macOS、Windows、麒麟三套目标系统的通知准时率长稳测试，至少覆盖睡眠唤醒、系统时区变更和应用隐藏状态。
4. 多显示器热插拔后的窗口可见性恢复。
