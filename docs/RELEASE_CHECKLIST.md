...
| 场景 | macOS 13/14/15 |
|---|---:|
| 首次启动、透明窗口、浅/深色 | 待签名构建 |
| Liquid Glass、交互反馈、降低透明度、高对比度及无模糊回退 | 待真机 |
| 多显示器位置恢复与锁定 | 待真机 |
| 关闭转托盘、托盘退出、单实例 | 待真机 |
| 通知授权、睡眠唤醒、时区/DST | 待真机 |
| 通知快捷稍后与每日 3 次上限 | 待真机 |
| WidgetKit 小/中尺寸、透明度、5 秒内刷新 | 待签名构建 |
| 安装、升级、卸载与数据保留 | 待签名构建 |
...
3. macOS 通知准时率长稳测试，至少覆盖睡眠唤醒、系统时区变更和应用隐藏状态。
4. 多显示器热插拔后的窗口可见性恢复。
