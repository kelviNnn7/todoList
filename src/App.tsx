...
import { calendarBadge, dateKey, itemDate, itemOccurrenceForDate, itemsForDate, longDate, monthDays, sortItems, weekDays } from "./lib/calendar";
...
export default function App() {
  ...
  const systemDateKey = useRef(dateKey(calendarNow));
  const days = useMemo(() => viewMode === "week" ? weekDays(calendarAnchor) : monthDays(calendarAnchor), [calendarAnchor, viewMode]);

  const showToday = useCallback((today = new Date()) => {
    systemDateKey.current = dateKey(today);
    setCalendarNow(today);
    setSelectedDate(today);
    setCalendarAnchor(today);
  }, []);
  const syncSystemDate = useCallback((now = new Date()) => {
    if (systemDateKey.current !== dateKey(now)) {
      showToday(now);
      return;
    }
    setCalendarNow(now);
  ...
  useEffect(() => {
    ...
    listen<string>("deep-link", (payload) => {
      if (payload.endsWith("://today")) showToday();
    }).then((dispose) => { unlisten = dispose; }).catch(() => undefined);
    return () => unlisten?.();
  }, [showToday]);
  useEffect(() => {
    const syncWhenActive = () => {
      if (document.visibilityState === "visible") syncSystemDate();
    };
  ...
  function chooseView(next: CalendarViewMode) { setViewMode(next); showToday(); writeScopedValue(`calendarView.${expandedWindow ? "expanded" : "widget"}`, next); }
  async function toggleWindow() {
    const next = !expandedWindow; setExpandedWindow(next); setViewMode(savedView(next)); showToday();
  ...
  async function toggleDesktopWidget() {
    ...
    if (next && expandedWindow) { setExpandedWindow(false); setViewMode(savedView(false)); showToday(); if (isDesktopRuntime()) await invoke("set_window_mode", { expanded: false }); }
  ...
  return <main className={`app-shell ${expandedWindow ? "expanded" : ""} ${desktopWidget ? "desktop-widget" : ""} ${locked ? "position-locked" : ""} ${dragging ? "dragging" : ""}`} style={{ "--widget-opacity": appearanceOpacity / 100, "--font-scale": fontScale[appearanceFontLevel] } as React.CSSProperties}>
    ...
    <header className="topbar" onMouseDown={startWindowDrag}>
      <div className="date-heading"><span className="brand-line"><img className="app-glyph" src={appIcon} alt=""/>BluNote</span><h1>{longDate(calendarNow)}</h1></div>
    ...
    <section className={`calendar-panel ${viewMode}`}><div className="view-tabs" aria-label="日历视图"><button className={viewMode === "week" ? "active" : ""} aria-pressed={viewMode === "week"} onClick={() => chooseView("week")}>周</button><button className={viewMode === "month" ? "active" : ""} aria-pressed={viewMode === "month"} onClick={() => chooseView("month")}>月</button></div><div className="calendar-toolbar"><button onClick={() => navigateCalendar(-1)} aria-label={viewMode === "week" ? "上一周" : "上个月"}><ChevronLeft size={15}/></button><button className="calendar-title" onClick={() => showToday()}>{calendarTitle}</button><button onClick={() => navigateCalendar(1)} aria-label={viewMode === "week" ? "下一周" : "下个月"}><ChevronRight size={15}/></button></div><div className="calendar-grid">{days.map((day) => { const badge = calendarBadge(items, day, calendarNow); return <button key={dateKey(day)} className={`${isSameDay(day, selectedDate) ? "selected" : ""} ${isToday(day) ? "today" : ""} ${viewMode === "month" && !isSameMonth(day, calendarAnchor) ? "outside-month" : ""}`} onClick={() => setSelectedDate(day)}><span>{format(day, "EEE", { locale: zhCN })}</span><strong>{format(day, "d")}</strong><i className={badge.overdue ? "overdue" : undefined}>{badge.count > 0 && Math.min(badge.count, 9)}</i></button>; })}</div></section>
