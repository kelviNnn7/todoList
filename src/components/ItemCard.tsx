...
import { Bell, BellRing, ChevronDown, ChevronRight, Clock3, MapPin, Pencil, Repeat2, Trash2, Video } from "lucide-react";
...
export function ItemCard({ item, autoExpand = false, onChange, onDelete, onEdit }: Props) {
  ...
  const recurring = item.type === "task" && item.taskSchedule?.mode === "weekly";
  const toggleItem = () => onChange({ ...item, completed: !item.completed, subtasks: recurring ? item.subtasks : !item.completed ? item.subtasks.map((s) => ({ ...s, completed: true })) : item.subtasks, updatedAt: new Date().toISOString() });
  const toggleSubtask = (id: string) => {
    const subtasks = item.subtasks.map((subtask) => subtask.id === id ? { ...subtask, completed: !subtask.completed } : subtask);
    return onChange({ ...item, subtasks, completed: recurring ? item.completed : subtasks.length > 0 && subtasks.every((subtask) => subtask.completed), updatedAt: new Date().toISOString() });
  };
  ...
  return <article className={`item-card ${item.type} ${item.completed ? "completed" : ""} ${overdue ? "overdue" : ""}`}>
    <div className="item-main">
      ...
      <button className="item-content" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>
        ...
        <span className="item-meta"><Clock3 size={13}/>{when}{recurring && <><Repeat2 size={13}/>每周</>}{item.type === "meeting" && item.reminderMinutes != null && <><Bell size={13}/>{item.reminderMinutes} 分钟前</>}{item.type === "task" && item.reminderAt && <><Bell size={13}/>{reminderLabel}</>}{item.reminderStatus === "fired" && <em>待处理提醒</em>}{overdue && <em>已逾期</em>}</span>
      </button>
